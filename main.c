...
void main()
{
    ...
    top = NULL; // baþlangýçta null yapýyoruz
...
void push(int data)
{
    ...
    if (top == NULL) // bellek boþsa ilk veriyi ekliyoruz
    {
        top =(struct node *)malloc(1*sizeof(struct node)); // bellekten yer ayiriyoruz
        top->next = NULL; // top düðümün next hücresini null yapýyoruz
    ...
    else // bellekte veri var ise 
    {
        ...
        temp->next = top; // temp iþaretçinin next hücresinin top düðümü göstermesini saðlýyoruz 
        temp->data = data;// temp iþaretçinin datasýna gelen datayý ekliyoruz
        top = temp;// temp artýk top oldu
    }
    sayac++;
}
...
void pop()
{
    ...
    if (temp_top == NULL) // Stack boþ ise
    {
        printf("\n !!! Yiginda Eleman Yok");
        return;
    }
    else
        temp_top = temp_top->next; // Bir sonraki elemaný göster
...
void reset()
{
    ...
    {
        temp_top = top->next; //top düðümden bir önceki düðümü temp_top iþaretçisine ata
        free(top); // ilk elemani serbest birak
        top = temp_top; // temp_top artýk top isaretcisi olsun 
